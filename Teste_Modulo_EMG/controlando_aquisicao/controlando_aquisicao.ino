...
long frequencia = 100;                 //frequência de aquisição do sinal, neste caso, 1000 sinais por segundo
unsigned long atraso_us = (1000000.0/frequencia);//Passo da aquisição é de 1000us=1ms
int medida;
...
int filtro_horizontal=200; //intervalo de 200 valores não pode ter mais de 1 contração


void setup() {
  Serial.begin(115200);                 //bits por segundo
  Serial.println("Configuracao_ok!"); 
  tempo_anterior = micros();            //Retorna tempo em microsegundos desde que Arduino começou a executar o programa
  tempo_cruzamento_anterior=micros();
}
...
void loop() { 
    ...
    if (abs(tempo_atual - tempo_anterior) >= atraso_us) { //Quando a diferença de tempo for >= ao Passo, fazemos a coleta do dado 
      tempo_anterior = tempo_atual;       //Redefine o tempo anterior
